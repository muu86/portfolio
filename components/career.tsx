...
import { metadata as digitalNutritionMetadata } from "@/docs/digital-nutrition.mdx";
import { metadata as uniitechMetadata } from "@/docs/uniitech.mdx";
...
export function Career() {
  return (
    <section className="mt-20">
      <h1 className="p-4 text-xl font-bold">경력</h1>
      <div className="flex flex-col">
        <div>
          ...
          <div className="my-4 flex flex-col">
            {digitalNutritionMetadata.projects.map((project, index) => (
        ...
        <div className="border-y-2 border-y-black">
          <div className="flex gap-5 border-b-2 border-b-zinc-100 p-4 text-xl">
            <h1 className="w-32 font-bold">{uniitechMetadata.company}</h1>
            <p className="text-zinc-500">{uniitechMetadata.description}</p>
          </div>
          <div className="my-4 flex gap-5 px-4">
            <div className="flex flex-wrap gap-2">
              {uniitechMetadata.techStack.map((tech, index) => (
                <TooltipProvider key={index} delayDuration={0}>
                  <Tooltip>
                    <TooltipTrigger>
                      <div className="flex items-center gap-2 rounded-lg bg-zinc-100 p-2">
        ...
        <div className="mt-4 flex flex-col">
          {uniitechMetadata.projects.map((project, index) => (
            <Project key={index} title={project.name} descriptions={project.description} period={project.period} />
          ))}
        </div>
      </div>
